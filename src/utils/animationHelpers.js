...
export const handleParallax = (element, scrollProgress, options = {}) => {
  ...
  const moveDistance = Math.min(scrollProgress * distance * intensity, distance)
...
export const applyTiltEffect = (element, mouseX, mouseY, options = {}) => {
  ...
  const angleX = ((mouseY - elementCenterY) / elementRect.height) * rotateAmount
  const angleY = ((mouseX - elementCenterX) / elementRect.width) * -rotateAmount
 ...
 */
export const cursorFollowAnimation = (element, targetX, targetY, options = {}) => {
